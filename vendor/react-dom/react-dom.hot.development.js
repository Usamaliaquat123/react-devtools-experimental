...
var eventResponderContext = {
  dispatchEvent: function (possibleEventObject, listener, _ref) {
    ...
    if (target == null || type == null) {
      throw new Error('context.dispatchEvent: "target" and "type" fields on event object are required.');
    }
    {
  ...
  isPositionWithinTouchHitTarget: function (x, y) {
    validateResponderContext();
  ...
  isTargetDirectlyWithinEventComponent: function (target) {
    validateResponderContext();
    if (target != null) {
      var fiber = getClosestInstanceFromNode(target);
      while (fiber !== null) {
  ...
  isTargetWithinEventResponderScope: function (target) {
    ...
    if (target != null) {
      ...
      while (fiber !== null) {
        ...
        if (fiber.tag === EventComponent && fiber.stateNode.responder === responder) {
          return false;
        }
        fiber = fiber.return;
      }
  ...
  hasOwnership: function () {
    validateResponderContext();
    var responder = currentInstance.responder;
    return globalOwner === currentInstance || responderOwners.get(responder) === currentInstance;
  },
  ...
  requestResponderOwnership: function () {
    validateResponderContext();
    var eventComponentInstance = currentInstance;
  ...
  getFocusableElementsInScope: function () {
    ...
    var eventComponentInstance = currentInstance;
    var node = eventComponentInstance.currentFiber.child;
...
function releaseOwnershipForEventComponentInstance(eventComponentInstance) {
  var responder = eventComponentInstance.responder;
  var triggerOwnershipListenersWith = void 0;
...
function shouldSkipEventComponent(eventResponderInstance, propagatedEventResponders) {
  var responder = eventResponderInstance.responder;
  ...
  if (responderOwners.has(responder) && responderOwners.get(responder) !== eventResponderInstance) {
    return true;
  }
  return false;
}
  ...
  if (length > 0) {
    ...
    for (i = length; i-- > 0;) {
      ...
      if (eventListener !== undefined) {
        if (shouldSkipEventComponent(targetEventResponderInstance, propagatedEventResponders)) {
          continue;
        }
    ...
    for (i = 0; i < length; i++) {
      ...
      if (_eventListener !== undefined) {
        if (shouldSkipEventComponent(_targetEventResponderInstance, propagatedEventResponders)) {
          continue;
        }
...
function triggerOwnershipListeners(limitByResponder) {
...
function validateResponderContext() {
  (function () {
    if (!(currentEventQueue && currentInstance)) {
      throw ReactError('An event responder context was used outside of an event cycle. Use context.setTimeout() to use asynchronous responder context outside of event cycle .');
    }
  })();
}
...
function registerRootEventType(rootEventType, eventComponentInstance) {
  ...
  (function () {
    if (!!rootEventTypesSet.has(listeningName)) {
      throw ReactError('addRootEventTypes() found a duplicate root event type of "' + name + '". This might be because the event type exists in the event responder "rootEventTypes" array or because of a previous addRootEventTypes() using this root event type.');
    }
  })();
...
function assertValidProps(tag, props) {
  ...
  if (voidElementTags[tag]) {
    (function () {
      if (!((props.children == null || enableEventAPI && props.children.type.$$typeof === REACT_EVENT_TARGET_TYPE) && props.dangerouslySetInnerHTML == null)) {
        {
...
function areCompatibleTypesForHotReload(a, b) {
  ...
  switch (typeof a) {
    case 'string':
      return false;
    case 'function':
      if (a.prototype && a.prototype.isReactComponent || b.prototype && b.prototype.isReactComponent) {
        // TODO: what's the class behavior?
        return false;
      }
      return a.__debugIdentity !== undefined && a.__debugIdentity === b.__debugIdentity;
    case 'object':
...
function resolveTypeWithHotReload(type) {
  var debugIdentity = type.__debugIdentity;
...
var failedBoundaries = new WeakSet();
function markFailedBoundaryForHotReload(fiber) {
  failedBoundaries.add(fiber);
}

// TODO: unify in one API?
function scheduleUpdateForHotReload(root, updatedIdentities, invalidatedIdentities) {
  flushPassiveEffects();
  var affectedFibers = [];
  ...
  var hostNodes = [];
  affectedFibers.forEach(function (f) {
    if (!isFiberMounted(f)) {
      return;
    }
    var hostFibers = findAllCurrentHostFibers(f);
    hostFibers.forEach(function (hf) {
      return hostNodes.push(hf.stateNode);
    });
  });
  return {
    hostNodes: hostNodes
...
function recursivelyScheduleUpdates(fiber, updatedIdentities, invalidatedIdentities, affectedFibers) {
  if (fiber.type !== null) {
    ...
    if (failedBoundaries.has(fiber) || fiber.alternate !== null && failedBoundaries.has(fiber.alternate)) {
      ...
      fiber.return.memoizedProps = _assign({}, fiber.return.memoizedProps, {
    ...
    var unwrappedType = unwrapHotReloadableType(fiber.type);
...
function invalidateHooks(fiber) {
...
function findAllCurrentHostFibers(parent) {
  var fibers = [];
  var currentParent = findCurrentFiberUsingSlowPath(parent);
  if (!currentParent) {
    return fibers;
  ...
  while (fiber !== null) {
    ...
    if (list !== null) {
      ...
      while (dependency !== null) {
        ...
        var depContext = dependency.context;
        {
          depContext = resolveTypeWithHotReload(depContext);
        }
        if (depContext === context && (dependency.observedBits & changedBits) !== 0) {
...
function acceptsType(fiber, element) {
  {
...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function updateElement(returnFiber, current$$1, element, expirationTime) {
    if (current$$1 !== null && acceptsType(current$$1, element)) {
      // Move based on index
  ...
  function reconcileSingleElement(returnFiber, currentFirstChild, element, expirationTime) {
    ...
    while (child !== null) {
      ...
      if (child.key === key) {
        if (child.tag === Fragment ? element.type === REACT_FRAGMENT_TYPE : acceptsType(child, element)) {
...
function injectIntoDevTools(devToolsConfig) {
  ...
  return injectInternals(_assign({}, devToolsConfig, {
    ...
    scheduleUpdateForHotReload: scheduleUpdateForHotReload,
...
var ReactDOM = {
  ...
  __SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED: {
    ...
    Events: [getInstanceFromNode$1, getNodeFromInstance$1, getFiberCurrentPropsFromNode$1, injection.injectEventPluginsByName, eventNameDispatchConfigs, accumulateTwoPhaseDispatches, accumulateDirectDispatches, enqueueStateRestore, restoreStateIfNeeded, dispatchEvent, runEventsInBatch, flushPassiveEffects]
  }
};
