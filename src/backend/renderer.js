...
export function attach(
  ...
  function recordClearChildren(fiber) {
    const primaryFiber = getPrimaryFiber(fiber);
    const id = getFiberID(primaryFiber);
    const operation = new Uint32Array(2);
    operation[0] = TREE_OPERATION_CLEAR_CHILDREN;
    operation[1] = id;
    ...
    if (nextFiber.tag === SuspenseComponent) {
      ...
      if (prevDidTimeout) {
        if (nextDidTimeOut) {
          ...
          // ----------------
          // TODO: or should it be nextFallbackChildSet.alternate?
          // That one is sometimes null :-(
          // ----------------
